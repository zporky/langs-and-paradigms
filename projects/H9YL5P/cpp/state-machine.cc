...
int StateMachine::nextState(std::string event, std::string symbol) {
	if (getState() == FloatEnums::S0 && event == FloatEnums::sign) {
		...
		return 0;
		         ...
		         || getState() == FloatEnums::T2 
		         || getState() == FloatEnums::T3 
		         || getState() == FloatEnums::T4
		         || getState() == FloatEnums::T5)
	           && event == FloatEnums::exp) {
		appendNumber(symbol);
		setState(FloatEnums::E1);
		return 0;
	} else if (getState() == FloatEnums::E1 && event == FloatEnums::sign) {
		appendNumber(symbol);
		setState(FloatEnums::E2);
		return 0;
	} else if ((getState() == FloatEnums::E1 
		         || getState() == FloatEnums::E2 
		         || getState() == FloatEnums::E3 )
	           && (event == FloatEnums::decimal 
					|| event == FloatEnums::zero)) {
		appendNumber(symbol);
		setState(FloatEnums::E3);
		return 0;
	} 
	else {
		//invalid state
