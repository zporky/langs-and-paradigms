...
step([C|Cs], 'T1', O) :- isDecimalSymbol(C),
	step(Cs, 'T1', O1), O=[C|O1].
step([C|Cs], 'T1', O) :- isESymbol(C),
	step(Cs, 'T6', O1), O=[C|O1].
%% T2
step([], 'T2', []).
step([C|Cs], 'T2', O) :- isPointSymbol(C),
	step(Cs, 'T3', O1), O=[C|O1].
step([C|Cs], 'T2', O) :- isESymbol(C),
	step(Cs, 'T6', O1), O=[C|O1].
%% T3
step([], 'T3', []).
step([C|Cs], 'T3', O) :- isDecimalSymbol(C),
	step(Cs, 'T3', O1), O=[C|O1].
step([C|Cs], 'T3', O) :- isESymbol(C),
	step(Cs, 'T6', O1), O=[C|O1].
%% T4
step([], 'T4', []).
...
step([C|Cs], 'T4', O) :- isESymbol(C),
	step(Cs, 'T6', O1), O=[C|O1].
%% T5
step([], 'T5', []).
step([C|Cs], 'T5', O) :- isDecimalSymbol(C),
	step(Cs, 'T5', O1), O=[C|O1].
step([C|Cs], 'T5', O) :- isESymbol(C),
	step(Cs, 'T6', O1), O=[C|O1].
%% T6
step([C|Cs], 'T6', O) :- 
	isStartSymbol(C), step(Cs, 'T7', O1), minus_or_nothing(C, E), O=[E|O1].
step([C|Cs], 'T6', O) :- step(Cs, 'T7', O).
%% T7
step([], 'T7', []).
step([C|Cs], 'T7', O) :- isDecimalSymbol(C),
	step(Cs, 'T7', O1), O=[C|O1].
