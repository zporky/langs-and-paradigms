...
public class StateMachine {
	...
	String processInput(String symbol) {
		...
		} else if (symbol.equals("e") || symbol.equals("E")) {
			return FloatConstants.exp;
		} else {
			//unkown symbol...
	...
	int nextState(String event, String symbol) {
		...
		if (getState() == FloatConstants.S0 && event == FloatConstants.sign) {
			...
			return 0;
			         ...
			         || getState() == FloatConstants.T2 
			         || getState() == FloatConstants.T3 
			         || getState() == FloatConstants.T4
			         || getState() == FloatConstants.T5)
		           && event == FloatConstants.exp) {
			appendNumber(symbol);
			setState(FloatConstants.E1);
			return 0;
		} else if (getState() == FloatConstants.E1 && event == FloatConstants.sign) {
			appendNumber(symbol);
			setState(FloatConstants.E2);
			return 0;
		} else if ((getState() == FloatConstants.E1
						|| getState() == FloatConstants.E2
						|| getState() == FloatConstants.E3)
						&& (event == FloatConstants.decimal
						|| event == FloatConstants.zero)) {
			appendNumber(symbol);
			setState(FloatConstants.E3);
			return 0;
		}
		else {
			//invalid state
