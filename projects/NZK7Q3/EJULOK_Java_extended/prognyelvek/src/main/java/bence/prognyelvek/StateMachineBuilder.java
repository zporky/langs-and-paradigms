...
public class StateMachineBuilder {
    ...
    public static StateMachine<String, Character, Character> build() {
		...
		final SimpleState<Character, Character> t7 = new SimpleState<>("t7", true);
		...
		t1.addTransition(new Transition<>(isE, extendedCopyAction, t6));	//new

        t2.addTransition(new Transition<>(isP, pointProcessing, t3));
		t2.addTransition(new Transition<>(isE, extendedCopyAction, t6));	//new

        t3.addTransition(new Transition<>(isZorD, extendedCopyAction, t3));
		t3.addTransition(new Transition<>(isE, extendedCopyAction, t6));	//new

        t4.addTransition(new Transition<>(isZorD, extendedCopyAction, t4));
        t4.addTransition(new Transition<>(isP, pointProcessing, t5));
		t4.addTransition(new Transition<>(isE, extendedCopyAction, t6));	//new

        t5.addTransition(new Transition<>(isZorD, extendedCopyAction, t5));
		t5.addTransition(new Transition<>(isE, extendedCopyAction, t6));	//new

		//new
		t6.addTransition(new Transition<>(isS, shallowAction, t7));
        t6.addTransition(new Transition<>(alwaysTrue, noAction, t7));
		
		t7.addTransition(new Transition<>(isZorD, extendedCopyAction, t1));
